...
/// The configured default directory is the interface's alone: a flat listing is
/// more often a script's, and a script means the directory it ran from.
#[test]
fn flat_mode_ignores_the_default_dir_env_var() {
    let target = tree("default_dir_target");
    let cwd = Path::new(env!("CARGO_TARGET_TMPDIR")).join("default_dir_cwd");
    let _ = fs::remove_dir_all(&cwd);
    fs::create_dir_all(&cwd).unwrap();
    let env = [("CEPHDU_DEFAULT_DIR", target.to_str().unwrap())];

    for args in [&["-p"], &["-f"], &["-l"]] {
        let out = run_from(Some(&cwd), &env, args);
        assert!(out.success, "{}", out.stderr);
        assert_eq!(
            out.stdout, "",
            "{:?} listed the default directory rather than the cwd",
            args
        );
    }
}
